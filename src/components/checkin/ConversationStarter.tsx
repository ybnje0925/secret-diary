  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-2">
        <button onClick={() => onRegenerate("casual")} className="rounded-full border border-[#ead8c9] bg-white py-2 text-xs font-extrabold text-[#5a392a]">더 편하게</button>
        <button onClick={() => onRegenerate("polite")} className="rounded-full border border-[#ead8c9] bg-white py-2 text-xs font-extrabold text-[#5a392a]">더 정중하게</button>
        <button onClick={() => onRegenerate("short")} className="rounded-full border border-[#ead8c9] bg-white py-2 text-xs font-extrabold text-[#5a392a]">더 짧게</button>
      </div>
      ...
      {showComplete && (
        <section className="rounded-2xl border border-[#ead8c9] bg-[#fffaf3] p-4 shadow-soft">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button onClick={saveContact} className="rounded-full bg-[#d85b36] py-3 text-sm font-extrabold text-white">
              <CheckCircle2 className="mr-1 inline h-4 w-4" /> 네, 연락했어요
            </button>
            <button onClick={() => setShowComplete(false)} className="rounded-full border border-[#ead8c9] bg-white py-3 text-sm font-extrabold text-[#5a392a]">아직이에요</button>
          </div>
        </section>
      )}
    </div>
...
function StarterCard({ label, text, onCopy }: { label: string; text: string; onCopy: (text: string) => void }) {
  return (
    <article className="rounded-2xl border border-[#ead8c9] bg-[#fffaf3] p-4 shadow-soft">
      ...
      <p className="whitespace-pre-line text-[15px] leading-relaxed text-[#2f1b12]">“{text}”</p>
    </article>
  );
}
