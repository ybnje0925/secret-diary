...
export default function PersonInfoSection({ person, onEdit }: Props) {
  return (
    <section className="space-y-3">
      ...
      <button onClick={onEdit} className="mt-2 w-full rounded-full bg-[#d85b36] py-4 font-extrabold text-white">정보 수정하기</button>
    </section>
  );
}
...
function InfoRow({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-[#ead8c9] bg-[#fffaf3] p-4 shadow-soft">
      <p className="mb-2 flex items-center gap-2 text-sm font-extrabold text-[#8d5b45]"><span className="[&>svg]:h-4 [&>svg]:w-4">{icon}</span>{label}</p>
      <p className="whitespace-pre-line text-[15px] leading-relaxed text-[#2f1b12]">{value}</p>
    </div>
  );
}
