  ...
  return (
    <section>
      <div className="mb-2.5 flex items-center justify-between">
        <h2 className="text-[15px] font-semibold leading-[1.45] tracking-[-0.015em] text-[#2f1b12]">오늘 떠올려볼 사람</h2>
      ...
      >
        ...
        <blockquote className="mt-3 rounded-xl bg-white/70 px-3 py-2.5 text-center text-[14px] font-normal leading-[1.65] text-[#5a392a]">
          “{getRecentMemory(person).split(".")[0]}”
        </blockquote>
        <div className="mt-3 grid grid-cols-2 gap-2">
          <button onClick={() => onOpenPerson(person.id)} className="rounded-full border border-[#dfa98f] bg-white py-2.5 text-xs font-semibold text-[#c95735]">
            기억 보기
          </button>
          <button onClick={() => onStartCheckIn(person.id)} className="rounded-full bg-[#d85b36] py-2.5 text-xs font-semibold text-white shadow-[0_8px_18px_rgba(216,91,54,0.18)]">
